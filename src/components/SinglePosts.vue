...
const isLikeProcessing = ref(false);
const selectedComment = ref(null);
const isImagePreviewOpen = ref(false); // 自訂圖片預覽狀態
const previewImageUrl = ref(""); // 當前預覽的圖片 URL
  ...
  try {
    ...
    if (response.status === 200 && Array.isArray(response.data)) {
      comments.value = response.data.map((comment) => ({
        ...
        userLiked: comment.user_liked || false,
        replies: comment.replies || 0,
        poster_url: comment.poster_url || "", // 假設後端提供預覽圖 URL
      }));
      emit("loaded");
    } else {
      console.error("數據格式不正確:", response.data);
