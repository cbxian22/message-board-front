...
const adjustTextareaHeight = () => {
  nextTick(() => {
    if (textarea.value && textareabox.value) {
      ...
      textarea.value.style.height = `${textHeight}px`;
      let totalHeight = textHeight;
      if (fileUrl.value) totalHeight += 90;
      textareabox.value.style.height = `${totalHeight}px`;
      textarea.value.style.overflowY =
        textarea.value.scrollHeight > 100 ? "auto" : "hidden";
    }
  });
};

watch([newMessage, fileUrl], adjustTextareaHeight, { immediate: true });
...
// 處理 Shift + Enter 換行，Enter 發送
const handleKeyDown = (event) => {
  if (event.key === "Enter" && !event.shiftKey) {
    event.preventDefault(); // 阻止默認換行
    sendMessage(); // 直接發送
  }
...
const addOrUpdateMessage = (message) => {
  const existingMsg = messages.value.find((m) => m.id === message.id);
  if (existingMsg) Object.assign(existingMsg, message);
...
const handleFileUpload = (event) => {
  ...
  try {
    if (selectedFile.type.startsWith("image/")) {
      ...
      reader.onerror = () => message.error("圖片讀取失敗！");
      reader.readAsDataURL(selectedFile);
    } else if (selectedFile.type.startsWith("video/")) {
      fileUrl.value = URL.createObjectURL(selectedFile);
      adjustTextareaHeight();
...
const fetchFriendName = async () => {
  ...
  console.log("最終 friend.value:", friend.value);
};
...
const fetchCurrentUser = async () => {
  try {
    ...
    socket.value.on("connect", () => console.log("WebSocket 連接成功"));
    socket.value.on("connect_error", (err) =>
      console.log("WebSocket 連接錯誤:", err)
    );
    socket.value.on("receiveMessage", async (message) => {
...
@media (max-width: 500px) {
  ...
  .icon-name {
    padding: 5px 15px; /* 適配手機 */
  }

  .messages {
    flex: 1;
    overflow-y: hidden; /* 移除滾動 */
    padding: 5px;
    min-height: 0;
  }
  ...
  .message-input {
    font-size: 14px;
    padding: 5px;
    max-height: 100px; /* 限制 textarea 高度 */
    overflow-y: auto; /* textarea 內部滾動 */
  }
